...
def baixa_dados_externo():
    ...
    dir = './Test_Raizen/airflow_dags/down_base'
    try:
        os.makedirs(dir)
    ...
    try:
        for i, row in df_links.iterrows():
            link = row[0]
            tipo = row['tipo']
            urllib.request.urlretrieve(link, "./Test_Raizen/airflow_dags/down_base/dados_"+ tipo+".csv")
            print("Dados " + tipo + " Baixado")  
    except:
            print("Dados " + tipo + " Deu Ruim")  

    # Baixa arquivo pivotado apenas para saber a data da verção do arquivo
    urllib.request.urlretrieve("https://www.gov.br/anp/pt-br/centrais-de-conteudo/dados-estatisticos/de/vdpb/vendas-combustiveis-m3.xls/@@download/file/vendas-combustiveis-m3.xlsx", "./Test_Raizen/airflow_dags/down_base/vendas-combustiveis-m3.xlsx")
    print("Dados Pivotado Baixado") 

    book = openpyxl.load_workbook('./Test_Raizen/airflow_dags/down_base/vendas-combustiveis-m3.xlsx')
    sheet = book['Plan1']
...
def tratando_dados_save_bigdate(ti):
    ...
    DF_DERIVADO =pd.read_csv('./Test_Raizen/airflow_dags/down_base/dados_Derivados.csv', sep = ';');  
    ...
    DF_DISEL =pd.read_csv('./Test_Raizen/airflow_dags/down_base/dados_Disel.csv', sep = ';');
    ...
    ### Salvando no Excel
    dir = './Test_Raizen/airflow_dags/new_tables'
    try:
        os.makedirs(dir)
        print('diretorio criado')
    except:
    ...
    # criar workbook
    wb = Workbook('./Test_Raizen/airflow_dags/new_tables/big_table.xlsx')
    wb.save('./Test_Raizen/airflow_dags/new_tables/big_table.xlsx')
    wb = load_workbook('./Test_Raizen/airflow_dags/new_tables/big_table.xlsx')
...
def enviando_email(ti):
    ...
    with open('./Test_Raizen/filekey.key', 'rb') as filekey:
        key = filekey.read()
        fernet = Fernet(key)
    ...
    filename = "./Test_Raizen/airflow_dags/new_tables/big_table.xlsx"  # In same directory as script
...
def atualizando_lastversion(ti):
    ...
    lastversion.to_csv('./Test_Raizen/airflow_dags/lastversion.csv', sep = ';')
    return ('Concluido')
